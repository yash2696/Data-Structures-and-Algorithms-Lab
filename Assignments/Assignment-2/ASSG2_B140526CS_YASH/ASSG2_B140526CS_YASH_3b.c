...
void radixSort(struct STUDENT [], int, int);
void quickSort(struct STUDENT [], int, int, int);
int partition(struct STUDENT [], int, int, int);
...
int main()
{
	...
	radixSort(STUDENT_LIST,length,n);
	printf("+----------+------+------+\n");
...
void radixSort(struct STUDENT record[], int length, int n)
{
	int i;
	for(i=length-1;i>=0;i--)
	{
		quickSort(record,0,n-1,i);
	}
}

void quickSort(struct STUDENT record[], int start, int end,int pos)
{
	int index;
	if(start >= end)
	{
	...
	{
		index = partition(record,start,end,pos);
		quickSort(record,start,index-1,pos);
		quickSort(record,index+1,end,pos);
	}
}

int partition(struct STUDENT array[], int start, int end, int pos)
{
	int pIndex = start, i, key = array[end].roll_no[pos];
	for(i=start;i<end;i++)
	{
		if(key >= array[i].roll_no[pos])
		{
			swap(array, i, pIndex);
			pIndex++;
		}
